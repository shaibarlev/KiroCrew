...
function renderPopover(
  ...
  const props = (next: AutomationRecord | null) => (
  ...
  return {
    ...
    onChange,
    ...view,
    rerenderAutomation: (next: AutomationRecord | null) => view.rerender(props(next)),
  }
}
...
describe('SessionAutomationPopover', () => {
  ...
  it('can replace an exact terminal monitor with a new provider monitor', async () => {
    ;(api.monitorCreate as ReturnType<typeof vi.fn>).mockResolvedValue({
      ok: true, monitor: {},
    })
    renderPopover({
      ...activeMonitor,
      active: false,
      actionable: true,
      terminal: { outcome: 'user_stop', reason: 'user_stop', stoppedAt: 1_800_000_400 },
    })
